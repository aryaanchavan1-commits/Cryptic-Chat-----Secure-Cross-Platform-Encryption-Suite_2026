...
class SecureCryptoGUI:
    def __init__(self, root):
        ...
        # Title
        ttk.Label(root, text="🔐 Secure Crypto Suite 2026 BY aryan chavan ",
                  font=("Segoe UI", 24, "bold")).pack(pady=10)
        
        ttk.Label(root, text=f"Platform: {self.platform_info}",
        ...
        # Status
        self.status_label = ttk.Label(root, text="Ready", font=("Segoe UI", 10))
        self.status_label.pack(pady=10)
        
        # Footer
        ttk.Label(root, text="Cross-Platform Secure Communication | Windows | Linux | macOS",
    ...
    def open_text_window(self):
        ...
        ttk.Label(window, text="📝 Text Encryption/Decryption",
                  font=("Segoe UI", 18, "bold")).pack(pady=10)
        
        # Text Input
        ttk.Label(window, text="Enter Text:").pack(anchor="w", padx=20)
        text_input = tk.Text(window, height=6, font=("Consolas", 11))
    ...
    def open_file_window(self):
        ...
        ttk.Label(window, text="📁 File & Folder Encryption/Decryption",
                  font=("Segoe UI", 18, "bold")).pack(pady=10)
    ...
    def open_messaging_window(self):
        ...
        # Left: Contact List
        contact_list_frame = tk.LabelFrame(chat_tab, text="Chat with Contact", padx=10, pady=10)
        contact_list_frame.pack(side="left", fill="y", padx=10, pady=10)
        
        contact_list = tk.Listbox(contact_list_frame, width=25, height=15, font=("Segoe UI", 10))
        contact_list.pack(fill="both", expand=True)
        
        # Connection Settings
        conn_frame = tk.LabelFrame(chat_tab, text="Connection Settings", padx=10, pady=10)
        conn_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Label(conn_frame, text="Target IP:").grid(row=0, column=0, padx=5)
        ip_var = tk.StringVar(value="127.0.0.1")
        ttk.Entry(conn_frame, textvariable=ip_var, width=15).grid(row=0, column=1, padx=5)
        
        ttk.Label(conn_frame, text="Port:").grid(row=0, column=2, padx=5)
        port_var = tk.IntVar(value=5050)
        ttk.Entry(conn_frame, textvariable=port_var, width=8).grid(row=0, column=3, padx=5)
        
        ttk.Label(conn_frame, text="Password:").grid(row=0, column=4, padx=5)
        password_var = tk.StringVar()
        ttk.Entry(conn_frame, textvariable=password_var, show="*", width=20).grid(row=0, column=5, padx=5)
        
        # Chat Display
        chat_frame = tk.LabelFrame(chat_tab, text="Chat", padx=10, pady=10)
        ...
        # Chat Buttons
        chat_btn_frame = tk.Frame(chat_tab)
        chat_btn_frame.pack(fill="x", padx=10, pady=5)
        
        ttk.Button(chat_btn_frame, text="Send", command=send_message).pack(side="left", padx=5)
        ttk.Button(chat_btn_frame, text="Start Listening", command=start_listening).pack(side="left", padx=5)
        ttk.Button(chat_btn_frame, text="Stop Listening", command=stop_listening).pack(side="left", padx=5)
        ttk.Button(chat_btn_frame, text="Auto Port", command=auto_port).pack(side="left", padx=5)
        ttk.Button(chat_btn_frame, text="Clear Chat", command=lambda: chat_text.delete("1.0", tk.END)).pack(side="left", padx=5)
